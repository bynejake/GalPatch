...
{
	if (!BeforeV2LinkCallback && !AfterV2LinkCallback)
		return;

	if (const auto pV2Link = DetoursHelper::FindExportEx(hModule, "V2Link"))
	{
		...
		if (BeforeV2LinkCallback && BeforeV2LinkCallback(hModule))
		{
			BeforeV2LinkCallback = nullptr;
		}

		if (AfterV2LinkCallback)
		{
